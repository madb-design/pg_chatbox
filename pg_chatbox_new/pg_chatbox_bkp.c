...
extract_response_text(char *json)
{
    ...
    // Remove Markdown backticks and explanations
...
chatbox_query(PG_FUNCTION_ARGS)
{
    ...
    snprintf(prompt, sizeof(prompt),
        ...
        "If the query cannot be answered, say: CANNOT ANSWER.\n\n"
        "Database objects:\n%s\n"
        "Question:\n%s",
        schema, question);
...
chatbox_explain(PG_FUNCTION_ARGS)
{
    ...
    char prompt[4096];
    snprintf(prompt, sizeof(prompt),
        "Explain this PostgreSQL query in simple terms without Markdown:\n%s", sql);
